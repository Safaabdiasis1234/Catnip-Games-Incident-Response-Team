...
SUSPICIOUS_IPS = {"203.0.113.50", "103.21.244.0"}  # demo/test IP ranges are fine for coursework
...
def decide_category(alert: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ----------------------------
    # Category scoring (simple but “advanced enough”)
    # ----------------------------
    ...
    if successful_login and ip_rep in ("malicious", "suspicious"):
        ...
        ac_reasons.append("successful_login == True")
        ac_reasons.append(f"ip_reputation == {ip_rep}")
        decisions.append(Decision("Account Compromise", ac_score, ac_reasons))
    ...
    if ca_score > 0:
        ...
        decisions.append(Decision("Credential Attack (SSH Brute Force / Bot Logins)", min(ca_score, 95), ca_reasons))
    ...
    if phishing_flag:
        ph_score = 45
        ph_reasons.append("phishing_flag == True")
        if ip_rep == "malicious":
            ph_score += 15
    ...
    # Simple tags (useful later if you push this into TheHive)
    tag_map = {
